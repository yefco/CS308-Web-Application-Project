...
pub async fn upsert_item(pool: &PgPool, cart_id: i32, product_id: i32, quantity: i32) -> Result<(), sqlx::Error> {
    sqlx::query(
...
pub async fn set_quantity(pool: &PgPool, cart_id: i32, product_id: i32, quantity: i32) -> Result<bool, sqlx::Error> {
    sqlx::query(
...
pub async fn merge_guest_into_user(pool: &PgPool, session_id: &str, user_id: i32) -> Result<(), sqlx::Error> {
    let guest_cart_id: Option<i32> = sqlx::query_scalar(
        "SELECT cart_id FROM shopping_carts WHERE session_id = $1",
    )
    .bind(session_id)
    .fetch_optional(pool)
    .await?;
